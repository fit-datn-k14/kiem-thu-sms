...
class SmsSingleController extends Controller
{
    ...
    public function getList($page = null){
        ...
        foreach ($results as $result) {
            ...
            $smsSingle = $this->smsModel->getSmsSingleByDate($result['id']);
            $data['contacts'][] = [
                'id' => $result['id'],
                ...
                'sms_single_is_sent' => $smsSingle['is_sent'],
                'sms_single_content' => $smsSingle['content'],
    ...
    public function writeSmsSingle() {
        $json = [];
        if(!Request::has('content') || !Request::has('contact_id')) {
        ...
        if(!isset($json['error'])) {
            ...
            $smsSingle = $this->smsModel->getSmsSingleByDate($contactId);
            $isAdd = true;
            if ($smsSingle) {
                $isAdd = false;
            }

            $contentSave = convert_vi_to_en(html_decode($smsPostContent));
            ...
            if ($smsLength > $smsNo3) {
                ...
                $json['data'] = $smsSingle['content'];
            } else {
                $data = [
                    'contact_id' => $contactId,
                    'content' => $contentSave,
                    'total_sms' => $contentSave ? $totalSmsLog : 0,
                ];

                if (!$isAdd) {
                    if ($smsSingle['content'] != $contentSave) {
                        $this->smsModel->writeSmsContentSingle($isAdd, $data);
                    }
                } else {
                    if ($contentSave) {
                        $this->smsModel->writeSmsContentSingle($isAdd, $data);
    ...
    public function sendSms() {
        ...
        if(!isset($json['error'])) {
            ...
            if ($smsConfig['total_sms_remain']) {
                ...
                $totalSmsSending = $this->smsModel->getSmsTotalSingle($contactIds);
                ...
                if ($smsConfig['total_sms_remain'] >= $totalSmsSending) {
                    $canSendSms = true;

                    $smsSingles = $this->smsModel->getSmsSingles($contactIds);
                    foreach ($smsSingles as $smsSingle) {
                        $totalSmsLog = $smsSingle['total_sms'];
                        $contactId = $smsSingle['contact_id'];
                        $smsContent = $smsSingle['content'];
                        ...
                        $contentReadySend .= convert_vi_to_en(html_decode($smsContent));
                    ...
                    $this->smsModel->addSms($smsDataSent, true);
